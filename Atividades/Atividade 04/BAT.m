...
Dados_simu.a = 10^((log10(0.1))/(0.25*Dados_simu.iter));
...
if carregar == 0
    ...
    name = ['Morcegos0_' num2str(Dados_simu.N) '.mat'];
    save(name, 'aux');
    clear aux
end
...
if carregar == 1
    name = ['Morcegos0_' num2str(Dados_simu.N) '.mat'];
    load(name)
    Solucao.MORCEGOS = aux; clear aux name
end
if length(Solucao.MORCEGOS) ~= Dados_simu.N
    stop
end
% Limitações
...
if figura_final == 1
    ...
    legend('Melhor', 'Location','northwest');
...
% Amazenamento resultado
Final.Resultado = Solucao.MORCEGOS(Solucao.index(1),:);
